...
for (const animation of json.animations || []) {
  for (const channel of animation.channels || []) {
    ...
    // Crouch/kneel/fall poses legitimately shift the hips a little. The bug is
    // the separate source-frame offset from the locomotion pack, which appears
    // as the whole standing gait starting far from the rig's rest hip height.
    if (!knownSourceFrameOffsetClips.has(animation.name) || Math.abs(offset) < 0.35) continue;

    for (let i = 0; i < accessor.count; i += 1) {
      values.set(i, 2, values.get(i, 2) - offset);
    }
    refreshAccessorMinMax(accessor, values);
    changedClips.push({ clip: animation.name, offset: Number(offset.toFixed(4)) });
  }
}
...
console.log(JSON.stringify({
  glb: glbPath,
  armatureRootGrounded: true,
  hipsRestZ: Number(hipsRestZ.toFixed(4)),
  correctedHipsTracks: changedClips,
}, null, 2));
...
function accessorView(accessor) {
  if (accessor.componentType !== 5126 || accessor.type !== 'VEC3') {
    throw new Error(`Expected FLOAT VEC3 accessor, saw componentType=${accessor.componentType} type=${accessor.type}`);
  }
...
function refreshAccessorMinMax(accessor, values) {
  ...
  for (let i = 0; i < accessor.count; i += 1) {
    for (let component = 0; component < 3; component += 1) {
      const value = values.get(i, component);
...
function encodeGlb(nextJson, nextBin, originalChunks) {
  ...
  if (extraChunks.length) {
    throw new Error('normalize-darwin-glb does not preserve unexpected extra GLB chunks.');
  }
  return out;
}
