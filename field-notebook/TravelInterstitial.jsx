...
export function TravelInterstitial() {
  ...
  return (
    <div className="pointer-events-auto fixed inset-0 z-40 flex items-center justify-center bg-stone-950/72 p-4 text-amber-50 backdrop-blur-sm">
      <section className="relative w-[min(44rem,calc(100vw-2rem))] overflow-hidden rounded-md border border-amber-200/35 bg-[#ead9ad] text-stone-950 shadow-2xl">
        ...
        <div className="relative grid gap-4 p-5 sm:grid-cols-[1fr_12rem] sm:p-6">
          <div>
          ...
          <div className="rounded border border-amber-900/25 bg-stone-950/10 p-3">
            ...
            <div className="mt-3 grid gap-1 text-xs text-stone-700">
              <span>{card?.estimatedMinutes || transition.minutes || 0} minutes</span>
              <span>Fatigue +{card?.fatigueDelta || transition.fatigue || 0}</span>
              <span>{card?.terrainType || zone.biome}</span>
            </div>
          </div>
          <div className="sm:col-span-2">
            ...
            <div className="mt-3 grid gap-1 text-xs text-stone-700 sm:grid-cols-3">
              ...
              <span>Route: {card?.routeLabel || 'local path'}</span>
