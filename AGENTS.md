...
- The reusable dry-region PBR baseline is documented in
  `docs/terrain-graphics.md`. Its reference implementation is Post Scrub Rise:
  use `layeredDryPbrTerrain.js`, packed linear NRH maps generated by
  `scripts/build-floreana-nrh-textures.mjs`, and region-owned layer config.
- Never manually decode an albedo already marked `THREE.SRGBColorSpace`.
  Remap roughness into plausible per-material ranges instead of treating source
  grayscale as an absolute physical value.
- Dry-zone raised detail should use `dryVolcanicLitter.js` and deterministic
  instanced transform variation from `scatter.js`; texture handles grit, litter
  handles small visible pieces, and obstacle data handles collision-scale rocks.
- Water/wading is height-driven. `WATER_LEVEL` and `WADE_DEPTH` live in
  ...
  from terrain depth.
- Current texture registry, packed-channel loaders, and color-space behavior live in
  `three-game/world/regions/materials/pbrTerrainTextures.js`.
- Post Office Bay detail now renders through `world/ecology/postOfficeBay.js`.
  Its visible basalt blocks must continue to come from
  `getPostOfficeBayBasaltBlocks()`, the same layout used to derive collision.
