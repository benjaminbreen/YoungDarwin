...
// Animated underwater life. Small, slow shoals travel broad routes beyond the
// wading beach instead of orbiting tight points around the player. Mantas
// patrol the deep drop-off, so spotting one stays an event.
function buildSwimmers() {
  ...
  // Both low-poly fish exports share a diagonal bind-pose forward axis. This
  // yaw aligns their actual nose (rather than their file axes) with +Z, which
  // is the forward direction used by ReefSwimmers' path controller.
  const LOW_POLY_FISH_FORWARD = [0, 0.9, 0];
  const LOW_POLY_FISH_ROOT_MOTION = ['Armature_7'];
  return {
    schools: [
      {
        id: 'ring-school',
        ...
        scale: [0.14, 0.2],
        motion: 'shoal',
        baseRotation: LOW_POLY_FISH_FORWARD,
        stripRootMotionNodes: LOW_POLY_FISH_ROOT_MOTION,
        bank: 0.025,
        maxPitch: 0.055,
      ...
      },
      {
        ...
        bank: 0.024,
        maxPitch: 0.05,
        verticalWander: 0.024,
        timeScale: 0.46,
        startleRadius: 4.5,
        startlePush: 1.4,
        startleSpeedBoost: 0.4,
        startleBank: 0.07,
      },
      {
        id: 'garden-school',
